...
model {
  ...
  sigma_0 ~ gamma(2,0.1);//normal(0,0.01);
  beta_gender ~ normal(0,1);

  for (i in 1:N){
//  RT_z[i] ~ normal(gamma_0[subject[i],item[i]] + beta_gender * gender[i] + beta_condition * condition[i] + beta_gendercondition * gender[i] * condition[i], sigma);
  RT_z[i] ~ normal(beta_0 + gammaz_0s[subject[i]]*sigma_0s + gammaz_0i[item[i]]*sigma_0i + beta_gender * gender[i], sigma_0);
  }
}
